...
def plot_artifact_distribution(
    artifact_data: pd.DataFrame, save_path: Optional[Path] = None
) -> None:
    """
    ...
    for artifact_type in artifact_data["type"].unique():
        ...
        axes[0, 1].scatter(
            data["length_cm"], data["width_cm"], label=artifact_type, alpha=0.6, s=50
        )
...
def plot_terrain_analysis(
    ...
    # Structures overlay
    axes[1].imshow(elevation, cmap="gray", extent=extent, aspect="auto", alpha=0.5)
...
def plot_geophysical_survey(
    ...
    # GPR anomalies
    axes[0, 1].imshow(anomalies["gpr"], cmap="binary", aspect="auto")
    ...
    axes[1, 1].imshow(combined_mag, cmap="RdBu_r", aspect="auto", vmin=-1, vmax=1)
...
def plot_site_comparison(
    site_features: dict[str, pd.DataFrame], save_path: Optional[Path] = None
) -> None:
    """
    ...
    avg_sizes = {
        site: df["length_cm"].mean() if "length_cm" in df else 0
        ...
        if len(structure_coords[0]) > 0:
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker={"size": 3, "color": "red", "opacity": 0.8},
                    name="Structures",
                )
            )
...
def plot_temporal_analysis(
    sites_by_period: dict[str, list[str]],
    ...
    ax.set_title("Temporal Distribution of Archaeological Sites", fontsize=14, fontweight="bold")
