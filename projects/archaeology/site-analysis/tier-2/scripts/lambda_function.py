...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not key.endswith('.csv'):
            ...
            return {
                'statusCode': 200,
                'body': {'message': 'Skipped non-CSV file'}
            }
        ...
        print(f"Downloading artifact data...")
        ...
        # Classify artifacts
        print(f"Classifying artifacts...")
        df_classified = classify_artifacts(df)

        # Calculate morphometric indices
        print(f"Calculating morphometric indices...")
...
def download_csv_from_s3(bucket: str, key: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to download {key} from {bucket}: {str(e)}")
...
def calculate_morphometrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['shape_index'] = np.minimum(df['length'], df['width']) / np.maximum(df['length'], df['width'])
...
def analyze_spatial_distribution(df: pd.DataFrame) -> Dict:
    ...
    spatial_summary = {
        ...
        'spread': {
            'lat_std': float(df['gps_lat'].std()),
            'lon_std': float(df['gps_lon'].std())
        },
        'clusters_by_type': {}
    }
...
def upload_json_to_s3(data: Dict, bucket: str, key: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to upload JSON to s3://{bucket}/{key}: {str(e)}")
...
def upload_dataframe_to_s3(df: pd.DataFrame, bucket: str, key: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to upload CSV to s3://{bucket}/{key}: {str(e)}")
