...
def query_by_site_and_period(table_name: str, site_id: str, period: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        if period:
            ...
            response = table.scan(
                FilterExpression=Attr('site_id').eq(site_id)
            )
            items = response['Items']
...
def query_by_location(
    ...
    print(f"Querying artifacts in bounding box:")
...
def display_results(items: List[Dict], max_rows: int = 20) -> None:
    ...
    display_cols = [
        'artifact_id', 'site_id', 'artifact_type', 'artifact_subtype',
        'material', 'period', 'length', 'width', 'weight',
        'gps_lat', 'gps_lon', 'classification_confidence'
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Query archaeological artifact data from DynamoDB'
    )
    parser.add_argument(
        '--table',
    ...
    parser.add_argument(
        '--type',
        help='Filter by artifact type (pottery, lithic, bone, coin, architecture)'
    )
    parser.add_argument(
        '--period',
        help='Filter by period (Neolithic, Bronze Age, etc.)'
    )
    parser.add_argument(
        '--site',
