...
def upload_to_s3(
    local_path: str, bucket: str, s3_key: str, region: str, show_progress: bool = True
) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Upload archaeological artifact data to S3")
    parser.add_argument("--bucket", help="S3 bucket name (overrides .env)")
    parser.add_argument("--site-id", default="SITE_A", help="Site identifier (default: SITE_A)")
    parser.add_argument(
        "--num-artifacts",
        type=int,
        default=100,
    ...
    parser.add_argument("--data-path", help="Path to local artifact CSV file")
    parser.add_argument(
        "--skip-upload", action="store_true", help="Generate data but skip S3 upload"
    )
    ...
    try:
        ...
        upload_to_s3(str(csv_path), config["bucket"], f"raw/{csv_path.name}", config["region"])
        ...
        summary = {
            ...
            "artifact_summary": {
                "by_type": df.groupby("artifact_type")
                .agg(
                    {
                        "artifact_id": "count",
                        "length": ["mean", "std"],
                ...
                .agg({"artifact_id": "count", "dating_value": ["min", "max", "mean"]})
                .to_dict(),
            },
        }
