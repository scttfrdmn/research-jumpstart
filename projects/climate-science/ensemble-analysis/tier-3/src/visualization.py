...
def plot_ensemble_timeseries(
    ...
    if show_individual:
        for model in ensemble.model.values:
            ...
            ax.plot(
                time, model_data.values, alpha=0.3, linewidth=1.0, color="gray", label="_nolegend_"
            )

    # Plot ensemble mean
    ax.plot(time, ens_mean.values, color="black", linewidth=2.5, label="Ensemble Mean", zorder=10)
...
def plot_model_agreement(
    ...
    # Plot range
    ax2.fill_between(
        time, ens_min.values, ens_max.values, alpha=0.2, color="gray", label="Full Range"
    )

    # Plot IQR
    ax2.fill_between(
        time, p25.values, p75.values, alpha=0.4, color="blue", label="Interquartile Range"
    )
...
def plot_scenario_comparison(
    ...
    for scenario_name, data in scenarios.items():
        ...
        ax.plot(data.time.values, data.values, label=scenario_name, linewidth=2.5, color=color)
...
def create_summary_figure(
    ...
    if spatial_data is not None:
        ...
        spatial_data.plot(
            ax=ax3, cmap="RdBu_r", transform=ccrs.PlateCarree(), cbar_kwargs={"shrink": 0.8}
        )
