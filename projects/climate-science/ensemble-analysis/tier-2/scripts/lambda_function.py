    ...
    try:
        ...
        try:
            if NETCDF_AVAILABLE and s3_key.endswith((".nc", ".nc4")):
                result = process_netcdf_file(file_data, s3_key, process_mode)
            else:
        ...
        try:
            ...
            error_msg = f"Failed to upload results: {e!s}"
            logger.error(error_msg)
            return error_response(error_msg, s3_key)
...
if __name__ == "__main__":
    ...
    test_event = {
        "Records": [
            {"s3": {"bucket": {"name": "climate-data-test"}, "object": {"key": "raw/test_data.nc"}}}
        ]
    }
