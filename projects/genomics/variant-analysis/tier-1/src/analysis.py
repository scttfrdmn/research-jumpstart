...
def build_cnn_variant_caller(
    input_shape: tuple[int, int, int] = (221, 100, 7), architecture: str = "resnet"
) -> keras.Model:
    """
...
def train_variant_caller(
    ...
    """
    # Build model
    model = build_cnn_variant_caller(input_shape=X_train.shape[1:], architecture=architecture)
...
def compare_with_truth_set(
    predicted_variants: pd.DataFrame, truth_vcf_path: Path, chromosome: str, tolerance: int = 0
) -> dict[str, any]:
    """
...
def save_variants_to_vcf(
    variants: pd.DataFrame, output_path: Path, sample_id: str, reference_genome: str = "GRCh37"
):
    """
