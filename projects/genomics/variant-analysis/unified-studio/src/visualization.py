...
def plot_manhattan(
    ...
    # Sort chromosomes
    chrom_order = sorted(plot_df['CHROM'].unique(), key=lambda x: (
        int(x) if x.isdigit() else (23 if x == 'X' else (24 if x == 'Y' else 25))
    ))

    # Assign colors
    colors = ['#1f77b4', '#ff7f0e']
    plot_df['color'] = [colors[i % 2] for i, chrom in enumerate(chrom_order)
    ...
    for chrom in chrom_order:
        ...
        ax.scatter(
            chrom_df['cum_pos'],
            chrom_df[p_col],
            c=chrom_df['color'].iloc[0],
            s=10,
            alpha=0.6
        )

    # Add significance threshold
    ax.axhline(sig_threshold, color='red', linestyle='--', linewidth=1,
    ...
    ax.set_title('Manhattan Plot - Variant Quality Across Genome',
...
def plot_variant_density(
    ...
    # Get chromosomes
    chroms = sorted(df['CHROM'].unique(), key=lambda x: (
        int(x) if x.isdigit() else (23 if x == 'X' else (24 if x == 'Y' else 25))
    ))
    ...
    for idx, chrom in enumerate(chroms):
        ...
        # Plot
        axes[idx].bar(edges[:-1] / 1e6, counts, width=(bin_size / 1e6),
...
def plot_quality_distribution(
    ...
    if 'QUAL' in df.columns:
        ...
        axes[0, 0].axvline(qual_data.mean(), color='red', linestyle='--',
                          label=f'Mean: {qual_data.mean():.1f}')
        axes[0, 0].axvline(qual_data.median(), color='green', linestyle='--',
    ...
    if 'DP' in df.columns:
        ...
        axes[0, 1].axvline(dp_data.mean(), color='red', linestyle='--',
                          label=f'Mean: {dp_data.mean():.1f}')
        axes[0, 1].axvline(dp_data.median(), color='green', linestyle='--',
                          label=f'Median: {dp_data.median():.1f}')
    ...
    if 'TYPE' in df.columns:
        ...
        axes[1, 1].bar(range(len(type_counts)), type_counts.values,
                      tick_label=type_counts.index, edgecolor='black', alpha=0.7)
...
def plot_allele_frequency_spectrum(
    ...
    ax.text(0.02, 0.95, f'Rare (AF<0.01): {rare:,}',
           transform=ax.transAxes, verticalalignment='top',
           bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    ax.text(0.02, 0.88, f'Common (AF≥0.05): {common:,}',
           transform=ax.transAxes, verticalalignment='top',
...
def plot_chromosome_summary(
    ...
    # Sort chromosomes
    chrom_order = sorted(chrom_counts.index, key=lambda x: (
        int(x) if x.isdigit() else (23 if x == 'X' else (24 if x == 'Y' else 25))
    ))
    ...
    colors = ['#1f77b4' if i % 2 == 0 else '#ff7f0e'
             for i in range(len(chrom_counts))]
