...
def list_1000genomes(
    phase="phase3", chromosome="chr22", data_type="vcf", max_results=20, anonymous=True
):
    """
...
def download_sample_vcf(chromosome="chr22", output_dir="data", phase="phase3", anonymous=True):
    """
    ...
    files = list_1000genomes(
        phase=phase, chromosome=chromosome, data_type="vcf", max_results=1, anonymous=anonymous
    )
    ...
    print("Note: VCF files are large, this may take several minutes...")
...
def list_tcga_data(
    project="TCGA-BRCA", data_category="Transcriptome Profiling", max_results=20, anonymous=True
):
    """
...
def list_gnomad_data(version="v3", data_type="vcf", max_results=20, anonymous=True):
    """
...
if __name__ == "__main__":
    ...
    genomes_files = list_1000genomes(
        phase="phase3", chromosome="chr22", data_type="vcf", max_results=5
    )
    if genomes_files:
        print("\nSample VCF files:")
    ...
    print("-" * 70)
    tcga_files = list_tcga_data(project="TCGA-BRCA", max_results=5)
    if tcga_files:
        print("\nSample TCGA files:")
    ...
    print("\n\n4. Example: gnomAD")
    print("-" * 70)
    gnomad_files = list_gnomad_data(version="v3", data_type="vcf", max_results=5)
    if gnomad_files:
        print("\nSample gnomAD files:")
