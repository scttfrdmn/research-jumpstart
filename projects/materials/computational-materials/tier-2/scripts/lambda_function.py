...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            error_msg = f"Failed to store in DynamoDB: {e!s}"
            logger.error(error_msg)
            return error_response(error_msg, s3_key)
...
def process_cif_file(file_data, filename):
    ...
    if properties["volume"] > 0 and atom_count > 0:
        ...
        properties["density"] = round(
            (avg_mass * atom_count) / (properties["volume"] * 1e-24) * 1.66054, 2
        )
    else:
...
def process_poscar_file(file_data, filename):
    ...
    try:
        ...
        if properties["volume"] > 0:
            ...
            properties["density"] = round(
                (avg_mass * properties["num_atoms"]) / (properties["volume"] * 1e-24) * 1.66054, 2
            )
        else:
            properties["density"] = 0.0
        ...
        properties["crystal_system"] = determine_crystal_system(
            ...
            90,
            90,
            90,  # Simplified - assumes orthogonal
        )
...
def determine_crystal_system(a, b, c, alpha, beta, gamma, tol=0.1):
    ...
    if (
        approx_equal(a, b)
        and approx_equal(b, c)
        and approx_equal(alpha, 90)
        and approx_equal(beta, 90)
        and approx_equal(gamma, 90)
    ):
        return "cubic"
