...
def query_participant(table_name, participant_id):
    ...
    response = table.query(KeyConditionExpression=Key("participant_id").eq(participant_id))
...
def query_by_task(table_name, task_type):
    ...
    response = table.scan(FilterExpression=Attr("task_type").eq(task_type))
...
def query_by_performance(table_name, min_accuracy=None, max_rt=None):
    ...
    print("Querying participants by performance...")
...
def display_participant_details(items):
    ...
    for item in items:
        ...
        elif item["task_type"] == "learning" and "learning_rate" in item:
            print("\nLearning:")
...
Examples:
  ...
  python query_results.py --table BehavioralAnalysis --export results.csv
    ...
    parser.add_argument(
        "--table",
        default="BehavioralAnalysis",
        help="DynamoDB table name (default: BehavioralAnalysis)",
    )
    parser.add_argument("--participant", help="Query specific participant ID")
    parser.add_argument("--task", help="Query specific task type (stroop, decision, learning)")
    parser.add_argument("--min-accuracy", type=float, help="Minimum accuracy filter")
    parser.add_argument("--max-rt", type=float, help="Maximum mean RT filter (ms)")
    parser.add_argument("--export", help="Export results to CSV file")
    parser.add_argument("--json", action="store_true", help="Display results as JSON")
