...
def generate_sample_data(n_participants=10, output_dir='sample_data'):
    ...
    for i in range(1, n_participants + 1):
        ...
        tasks = np.random.choice(['stroop', 'decision', 'learning'],
...
Examples:
  ...
  python upload_to_s3.py --bucket behavioral-data-12345 --generate-sample --n-participants 20
    ...
    parser.add_argument('--generate-sample', action='store_true',
                        help='Generate sample behavioral data')
    ...
    parser.add_argument('--s3-prefix', default='raw/',
                        help='S3 prefix (folder) for uploaded files (default: raw/)')
    ...
    if args.generate_sample:
        ...
        files = generate_sample_data(args.n_participants, output_dir)
    ...
    print(f"\nNext steps:")
    print(f"  1. Lambda function will process files automatically (if configured)")
    print(f"  2. Or invoke Lambda manually with scripts/invoke_lambda.py")
    print(f"  3. Query results with scripts/query_results.py")
