...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Store in DynamoDB
        table_name = context.function_name if hasattr(context, 'function_name') else 'BehavioralAnalysis'
...
def analyze_stroop(df):
    ...
    if len(congruent) > 0 and len(incongruent) > 0:
        ...
        results['stroop_effect_accuracy'] = results['accuracy_congruent'] - results['accuracy_incongruent']

        # Statistical test
        try:
            t_stat, p_value = stats.ttest_ind(
                incongruent['rt'].values,
                congruent['rt'].values
            )
...
def analyze_decision(df):
    ...
    if len(easy) > 0 and len(hard) > 0:
        ...
        results['difficulty_effect_accuracy'] = float(easy['accuracy'].mean() - hard['accuracy'].mean())
...
if __name__ == '__main__':
    ...
    test_event = {
        'bucket': 'behavioral-data-test',
        'key': 'raw/sub001_stroop.csv'
    }
