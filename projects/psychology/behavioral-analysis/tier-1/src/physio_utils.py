...
def compute_hrv(
    ...
    freqs, psd = signal.welch(rr_intervals, fs=1000/np.mean(rr_intervals), nperseg=min(256, len(rr_intervals)))
...
def extract_scr_features(
    ...
    if len(peaks) > 0:
        ...
        features['scr_peak_rate'] = len(peaks) / (len(gsr_signal) / sampling_rate)  # peaks per second
    else:
