...
def calculate_proper_motions(catalog):
    ...
    if "pmra" in catalog.colnames and "pmdec" in catalog.colnames:
        # Total proper motion
        features["pm_total"] = np.sqrt(catalog["pmra"] ** 2 + catalog["pmdec"] ** 2)

        # Proper motion direction
        features["pm_angle"] = np.arctan2(catalog["pmdec"], catalog["pmra"])
        ...
        if "parallax" in catalog.colnames:
            ...
            if "phot_g_mean_mag" in catalog.colnames:
                features["reduced_pm"] = (
                    catalog["phot_g_mean_mag"] + 5 * np.log10(features["pm_total"]) + 5
                )
...
def extract_morphology(catalog):
    ...
    if "petroR90_r" in catalog.colnames:
        ...
        # Concentration index
        if "petroR50_r" in catalog.colnames:
            features["concentration"] = catalog["petroR90_r"] / catalog["petroR50_r"]
...
def handle_missing_values(features, strategy="median"):
    ...
    imputed = pd.DataFrame(
        imputer.fit_transform(features), columns=features.columns, index=features.index
    )
