...
class TextCorpusUploader:
    ...
    def download_gutenberg_text(self, gutenberg_id: int, title: str) -> Optional[Path]:
        ...
        safe_title = "".join(c if c.isalnum() or c in (' ', '-') else ''
                            for c in title).strip().replace(' ', '-').lower()
    ...
    def upload_text_to_s3(self, filepath: Path, author: str, period: str,
                          genre: str, title: str) -> bool:
        """
    ...
    def upload_corpus(self, corpus_metadata: List[tuple]) -> Dict[str, int]:
        ...
        for gutenberg_id, author, title, period, genre in tqdm(corpus_metadata,
    ...
    def upload_local_corpus(self, corpus_dir: Path,
                           default_metadata: Dict[str, str]) -> Dict[str, int]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload historical text corpus to S3"
    )
    parser.add_argument(
        '--bucket',
        required=True,
        help='S3 bucket name (e.g., text-corpus-alice-12345)'
    )
    parser.add_argument(
        '--local-corpus',
        help='Path to local corpus directory (alternative to downloading)'
    )
    parser.add_argument(
        '--author',
        help='Default author for local corpus files'
    )
    parser.add_argument(
        '--period',
        help='Default period for local corpus files (Romantic, Victorian, etc.)'
    )
    parser.add_argument(
        '--genre',
        default='Novel',
        help='Default genre for local corpus files (default: Novel)'
    )

    args = parser.parse_args()

    # Initialize uploader
    uploader = TextCorpusUploader(
