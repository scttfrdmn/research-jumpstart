...
def plot_topic_distribution(
    ...
    _fig, axes = plt.subplots(1, min(n_topics, 5), figsize=figsize)
...
def plot_corpus_statistics(
    stats: dict, save_path: Optional[str] = None, figsize: tuple[int, int] = (12, 8)
) -> None:
    """
    ...
    # Total documents
    axes[0].bar(["Documents"], [stats.get("total_documents", 0)], alpha=0.7, edgecolor="black")
    ...
    # Total words
    axes[1].bar(
        ["Words"], [stats.get("total_words", 0)], alpha=0.7, edgecolor="black", color="orange"
    )
    ...
    axes[2].bar(
        ["Avg Length"],
        [stats.get("avg_document_length", 0)],
        alpha=0.7,
        edgecolor="black",
        color="green",
    )
    ...
    # Vocabulary size
    axes[3].bar(
        ["Vocabulary"], [stats.get("vocabulary_size", 0)], alpha=0.7, edgecolor="black", color="red"
    )
    ...
    for i in range(n):
        for j in range(i, n):
            if i == j:
                ...
                comparison = compare_texts(df.iloc[i][text_column], df.iloc[j][text_column])
