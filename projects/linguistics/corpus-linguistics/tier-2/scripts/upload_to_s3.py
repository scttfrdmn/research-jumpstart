...
Examples:
  ...
  python upload_to_s3.py --bucket my-bucket --corpus-dir ./corpus --dry-run
    ...
    parser.add_argument(
        '--bucket',
        required=True,
        help='S3 bucket name (e.g., linguistic-corpus-123)'
    )

    parser.add_argument(
        '--corpus-dir',
        required=True,
        help='Local directory containing corpus files'
    )
    ...
    parser.add_argument(
        '--genre',
        default=None,
        help='Override genre (default: detect from directory structure)'
    )
...
def upload_corpus(bucket: str, corpus_dir: str, file_pattern: str = '*.txt',
                  language_override: str = None, genre_override: str = None,
                  dry_run: bool = False):
    """
    ...
    if not dry_run:
        if not verify_bucket_exists(bucket):
            sys.exit(1)
    ...
    if dry_run:
        ...
        print(f"Run without --dry-run to actually upload files")
    else:
