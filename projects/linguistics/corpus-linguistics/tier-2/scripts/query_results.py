...
def scan_all_texts(table_name: str, filters: Dict = None, limit: int = 100) -> List[Dict]:
    """
...
def format_basic_summary(items: List[Dict]) -> str:
    ...
    for item in items:
        ...
        rows.append({
            ...
            'TTR': f"{lexical_div.get('ttr', 0):.3f}" if isinstance(lexical_div, dict) else 'N/A',
        })
