...
def display_results_table(results):
    ...
    # Table header
    print(f"{'Location':<20} {'Timestamp':<20} {'Load (MW)':<15} {'Renewable %':<15} {'Stability':<12} {'Status':<10}")
    print("-" * 120)
    ...
    for result in sorted_results:
        ...
        print(f"{location:<20} {timestamp:<20} {load_avg:>10.2f} MW  {renewable_pct:>10.2f}%  {stability:>10.3f}  {status:<10}")
...
def display_detailed_summary(results):
    ...
    renewable_pcts = [r['renewable_metrics']['renewable_percentage'] for r in results if 'renewable_metrics' in r]
    stability_scores = [r['power_quality']['stability_score'] for r in results if 'power_quality' in r]
    ...
    try:
        ...
        if format == 'json':
            ...
            with open(output_file, 'w', newline='') as f:
                ...
                fieldnames = [
                    'location', 'timestamp', 'alert_status',
                    'load_avg_mw', 'load_min_mw', 'load_max_mw',
                    'generation_avg_mw',
                    'renewable_penetration', 'renewable_percentage',
                    'voltage_avg_kv', 'voltage_min_kv', 'voltage_max_kv',
                    'frequency_avg_hz', 'frequency_min_hz', 'frequency_max_hz',
                    'power_factor_avg', 'stability_score', 'efficiency_score'
                ]
                ...
                for result in results:
                    row = {
                        ...
                        'renewable_penetration': result.get('renewable_metrics', {}).get('renewable_penetration', 0),
                        'renewable_percentage': result.get('renewable_metrics', {}).get('renewable_percentage', 0),
                        ...
                        'power_factor_avg': result.get('power_quality', {}).get('power_factor_avg', 0),
                        'stability_score': result.get('power_quality', {}).get('stability_score', 0),
                        'efficiency_score': result.get('power_quality', {}).get('efficiency_score', 0),
                    }
                    writer.writerow(row)
...
Examples:
  ...
  python query_results.py --table GridAnalysis --export results.json --format json
    ...
    parser.add_argument(
        '--table',
        default='GridAnalysis',
        help='DynamoDB table name (default: GridAnalysis)'
    )
    parser.add_argument(
        '--bucket',
        help='S3 bucket name for S3 results'
    )
    parser.add_argument(
        '--location',
        help='Filter by location (e.g., substation_001)'
    )
    parser.add_argument(
        '--limit',
        type=int,
    ...
    parser.add_argument(
        '--export',
        help='Export results to file (e.g., results.csv)'
    )
    parser.add_argument(
        '--format',
        choices=['csv', 'json'],
    ...
    parser.add_argument(
        '--summary',
        action='store_true',
        help='Display detailed statistical summary'
    )
    ...
    if results:
        ...
        print("\nNo results found.")
        print("\nTroubleshooting:")
        print("  1. Verify DynamoDB table exists: aws dynamodb describe-table --table-name GridAnalysis")
