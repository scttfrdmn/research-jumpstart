...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.error(f"Error processing file: {e!s}", exc_info=True)
...
def calculate_student_metrics(student_id: str, course_id: str, activity_df: pd.DataFrame) -> dict:
    """
    ...
    """
    # Grade metrics
    scores = activity_df[activity_df["submitted"]]["score"]
    ...
    if len(scores) > 0:
        ...
        if len(scores) > 2:
            ...
            slope, _intercept, _r_value, _p_value, _std_err = stats.linregress(x, scores)
    ...
    completion_rate = (
        (completed_assessments / total_assessments * 100) if total_assessments > 0 else 0.0
    )
    ...
    resource_score = min(
        100, (total_resources / (total_assessments * 5)) * 100
    )  # 5 views per task expected
...
def identify_risk_level(
    avg_grade: float, grade_trend: float, completion_rate: float, engagement_score: float
) -> tuple[str, list[str]]:
    """
...
def calculate_mastery_metrics(activity_df: pd.DataFrame) -> dict:
    ...
    """
    submitted_df = activity_df[activity_df["submitted"]].sort_values("assessment_number")
...
def store_results_dynamodb(results: list[dict]) -> int:
    ...
    for metrics in results:
        try:
            ...
            logger.error(f"Error storing item for student {metrics['student_id']}: {e!s}")
...
def save_results_to_s3(results: list[dict], bucket: str, source_key: str):
    ...
    try:
        ...
        logger.error(f"Error saving results to S3: {e!s}")
