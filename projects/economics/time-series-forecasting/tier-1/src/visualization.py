...
def plot_time_series(
    data: pd.DataFrame, title: str = "Economic Time Series", figsize: tuple = (14, 6)
) -> plt.Figure:
    """
...
def plot_correlation_heatmap(
    corr_matrix: pd.DataFrame, title: str = "Correlation Matrix", figsize: tuple = (12, 10)
) -> plt.Figure:
    """
...
def plot_forecast_error_variance_decomposition(
    fevd_data: pd.DataFrame, response_var: str, periods: int = 20, figsize: tuple = (14, 6)
) -> plt.Figure:
    """
    ...
    ax.set_title(
        f"Forecast Error Variance Decomposition: {response_var}",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
...
def create_interactive_dashboard(
    data: pd.DataFrame, title: str = "Economic Dashboard"
) -> go.Figure:
    """
    ...
    fig = make_subplots(
        rows=n_series, cols=1, subplot_titles=data.columns.tolist(), vertical_spacing=0.05
    )
...
def plot_model_performance(
    results: pd.DataFrame, metric: str = "RMSE", figsize: tuple = (12, 6)
) -> plt.Figure:
    """
    ...
    ax.set_title(f"Model Performance Comparison ({metric})", fontsize=14, fontweight="bold", pad=15)
...
def plot_residual_diagnostics(residuals: pd.Series, figsize: tuple = (14, 10)) -> plt.Figure:
    """
