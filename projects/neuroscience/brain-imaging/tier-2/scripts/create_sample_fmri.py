...
def create_sample_fmri(shape=(64, 64, 32, 150), noise_level=0.1, output_path='sample_fmri.nii.gz'):
    ...
    Returns:
        None (saves to file)
    """
    logger.info(f"Creating synthetic fMRI data...")
...
def create_fmri_with_task_activation(shape=(64, 64, 32, 150), output_path='sample_fmri_with_task.nii.gz'):
    """
...
Examples:
  ...
  python create_sample_fmri.py --task --output sample_fmri_task.nii.gz
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help='Noise level relative to signal (default: 0.1)'
    )
    parser.add_argument(
        '--task',
        action='store_true',
        help='Create fMRI with simulated task activation'
    )
    ...
    logger.info(f"Ready for upload to S3:")
