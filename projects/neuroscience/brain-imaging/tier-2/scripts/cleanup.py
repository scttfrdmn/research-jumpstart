...
def list_resources(
    input_bucket: str, output_bucket: str, function_name: str, role_name: str
) -> dict:
    """
...
def cleanup_all(
    input_bucket: str, output_bucket: str, function_name: str, role_name: str, confirm: bool = False
) -> bool:
    """
    ...
    logger.info(
        f"  Lambda function: {function_name} {'✓' if resources['lambda_function'] else '✗'}"
    )
    ...
    if resources["lambda_function"] and not delete_lambda_function(function_name):
        all_success = False

    # Delete IAM role
    if resources["iam_role"] and not delete_iam_role(role_name):
        all_success = False
...
WARNING: This script is destructive and cannot be undone!
    ...
    parser.add_argument("--iam-role", required=True, help="IAM role name")
    parser.add_argument(
        "--confirm", action="store_true", help="Skip confirmation prompt (DANGEROUS)"
    )
