...
def download_file_from_s3(bucket: str, s3_key: str, local_path: str) -> bool:
    ...
    try:
        ...
        s3.download_file(bucket, s3_key, local_path, Callback=ProgressPercentage(s3_key, file_size))
        ...
        logger.error(f"Failed to download {s3_key}: {e!s}")
        return False
...
def download_directory_from_s3(bucket: str, prefix: str, local_dir: str) -> dict:
    ...
    try:
        ...
        logger.error(f"Error downloading directory: {e!s}")
        return {"downloaded": 0, "failed": 0, "files": []}
...
def list_s3_results(bucket: str, prefix: str = "") -> dict:
    ...
    try:
        ...
        return {"motion_corrected": motion_corrected, "smoothed": smoothed, "other": other}

    except Exception as e:
        logger.error(f"Error listing results: {e!s}")
...
def load_fmri_data(local_path: str):
    ...
    try:
        ...
        logger.error(f"Error loading fMRI data: {e!s}")
        return None
...
Examples:
  ...
  python query_results.py --output-bucket fmri-output --local-path results/ --verify
    ...
    parser.add_argument("--output-bucket", required=False, help="S3 output bucket name")
    parser.add_argument(
        "--local-path", default="results", help="Local directory to download to (default: results/)"
    )
    parser.add_argument("--prefix", default="", help="S3 prefix to filter downloads")
    parser.add_argument(
        "--filter",
    ...
    parser.add_argument("--list", action="store_true", help="List results without downloading")
    parser.add_argument("--verify", action="store_true", help="Verify downloaded files")
    parser.add_argument("--load", help="Load and inspect a specific NIfTI file")
