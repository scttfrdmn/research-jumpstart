...
def calculate_traffic_metrics(
    road_network: gpd.GeoDataFrame, traffic_counts: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    if "capacity" in road_network.columns and "volume" in traffic_counts.columns:
        ...
        metrics["v_c_ratio"] = traffic_counts.apply(
            lambda row: row["volume"] / capacity_dict.get(row["road_id"], 1), axis=1
        )
...
def compute_commute_patterns(
    origin_dest_matrix: pd.DataFrame, road_network: gpd.GeoDataFrame
) -> pd.DataFrame:
    """
...
def calculate_avg_stop_spacing(transit_stops: gpd.GeoDataFrame) -> float:
    ...
    for route_id in transit_stops["route_id"].unique():
        route_stops = transit_stops[transit_stops["route_id"] == route_id].sort_values(
            "stop_sequence"
        )
