...
class EnvironmentalDataQuery:
    ...
    def query_by_alert_status(self, alert_status="critical", limit=50):
        ...
        try:
            response = self.table.scan(
                FilterExpression=Attr("alert_status").eq(alert_status), Limit=limit
            )
    ...
    def get_all_locations(self):
        ...
        try:
            response = self.table.scan(ProjectionExpression="location_id", Limit=1000)

            items = response.get("Items", [])
            locations = list({item["location_id"] for item in items})
            locations.sort()
    ...
    def get_statistics(self, location_id, days=7):
        ...
        stats = {
            ...
            "alerts": {"critical": 0, "warning": 0, "none": 0},
            "sensor_types": {},
        }
        ...
        for item in items:
            ...
            if sensor_type == "air":
                if "air_quality" not in stats:
                    stats["air_quality"] = {"aqi_values": [], "pm25_values": []}
                ...
                if "water_quality" not in stats:
                    stats["water_quality"] = {"wqi_values": [], "ph_values": []}
                metrics = item.get("calculated_metrics", {})
                params = item.get("parameters", {})
                if "wqi" in metrics:
...
def format_table(items, max_rows=20):
    ...
    # Table rows
    for _i, item in enumerate(items[:max_rows]):
...
Examples:
  ...
  python query_results.py --location station-01 --format json > results.json
    ...
    parser.add_argument("--location", help="Query specific location ID")
    ...
    parser.add_argument(
        "--alert-status", choices=["warning", "critical"], help="Filter by alert status"
    )
    parser.add_argument(
        "--sensor-type", choices=["air", "water", "weather"], help="Filter by sensor type"
    )
    parser.add_argument(
        "--statistics", action="store_true", help="Show statistics instead of raw results"
    )
    parser.add_argument("--list-locations", action="store_true", help="List all unique locations")
    parser.add_argument(
        "--format",
        choices=["table", "json"],
        default="table",
    ...
    try:
        query = EnvironmentalDataQuery(
            table_name=args.table, region=args.region, profile=args.profile
        )
