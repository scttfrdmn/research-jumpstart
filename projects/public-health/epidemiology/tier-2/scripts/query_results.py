...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Query disease surveillance results from DynamoDB")
    parser.add_argument(
        "--table-name",
        type=str,
    ...
    parser.add_argument(
        "--summaries-only", action="store_true", help="Only retrieve summary records"
    )
    parser.add_argument("--output", type=str, help="Output CSV file path")
    parser.add_argument(
        "--show-outbreak-alerts", action="store_true", help="Show only records with outbreak alerts"
    )
...
def query_by_disease(
    ...
    try:
        ...
        logger.error(f"Error querying by disease: {e!s}")
        return []
...
def query_by_region(
    ...
    try:
        ...
        logger.error(f"Error querying by region: {e!s}")
        return []
...
def scan_summaries(table, limit: int = 100) -> list[dict]:
    ...
    try:
        ...
        response = table.scan(FilterExpression=Attr("record_type").eq("summary"), Limit=limit)
        ...
        logger.error(f"Error scanning summaries: {e!s}")
        return []
...
def scan_with_filters(
    ...
    try:
        ...
        logger.error(f"Error scanning table: {e!s}")
        return []
...
def main():
    ...
    try:
        ...
        logger.error(f"Error connecting to DynamoDB: {e!s}")
        sys.exit(1)
