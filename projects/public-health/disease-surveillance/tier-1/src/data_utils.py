...
def load_covid_data(
    ...
    if cache_file.exists() and not force_download:
        print(f"Loading processed COVID-19 data from cache")
        df = pd.read_csv(cache_file, parse_dates=['Date'])
    else:
        print(f"Generating example COVID-19 surveillance data...")
...
def load_rsv_data(
    ...
    if cache_file.exists() and not force_download:
        print(f"Loading processed RSV data from cache")
        df = pd.read_csv(cache_file, parse_dates=['Date'])
    else:
        print(f"Generating example RSV surveillance data...")
...
def load_multi_disease_panel(
    diseases: List[str] = ['ILI', 'COVID-19', 'RSV'],
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    regions: Optional[List[str]] = None,
    force_download: bool = False
