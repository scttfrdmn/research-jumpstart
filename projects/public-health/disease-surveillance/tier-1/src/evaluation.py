...
def evaluate_peak_timing(
    y_true: np.ndarray, y_pred: np.ndarray, dates: Optional[pd.DatetimeIndex] = None
) -> dict[str, float]:
    """
...
def calculate_crps(y_true: np.ndarray, ensemble_forecasts: np.ndarray) -> float:
    """
...
def evaluate_multi_horizon_forecast(
    y_true: np.ndarray, y_pred: np.ndarray, horizon_names: Optional[list] = None
) -> pd.DataFrame:
    """
    ...
    for i in range(n_horizons):
        metrics = calculate_forecast_metrics(y_true[:, i], y_pred[:, i], prefix="")
