...
def execute_athena_query(query_string, bucket_name, output_location="athena-results/"):
    ...
    try:
        response = athena_client.start_query_execution(
            ...
            ResultConfiguration={"OutputLocation": f"s3://{bucket_name}/{output_location}"},
        )
...
Examples:
  ...
  python query_results.py --sample
    ...
    parser.add_argument("--list", action="store_true", help="List results in S3")
    parser.add_argument(
        "--output", default="./results", help="Output directory for downloaded files"
    )
    parser.add_argument("--athena", action="store_true", help="Use Athena for querying")
    parser.add_argument("--query", help="Athena SQL query to execute")
    parser.add_argument("--sample", action="store_true", help="Create sample result files")
    parser.add_argument("--prefix", default="results/", help="S3 prefix for results")
