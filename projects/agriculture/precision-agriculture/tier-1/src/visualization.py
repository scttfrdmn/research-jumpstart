...
def plot_confusion_map(
    ...
    scatter2 = axes[1].scatter(
        spatial_coords[:, 0], spatial_coords[:, 1], c=abs_errors, cmap="Reds", s=100, alpha=0.7
    )
...
def create_interactive_map(
    data: pd.DataFrame, lat_col: str, lon_col: str, value_col: str, title: str = "Interactive Map"
):
    """
    ...
    # Add heatmap layer
    heat_data = [[row[lat_col], row[lon_col], row[value_col]] for idx, row in data.iterrows()]
...
def plot_training_history(
    history, metrics: Optional[list[str]] = None, figsize: tuple[int, int] = (14, 5)
) -> plt.Figure:
    """
...
def plot_ndvi_evolution(
    ...
    # Mark key dates
    ax.axvline(
        phenology["greenup_date"], color="green", linestyle="--", label="Green-up", alpha=0.7
    )
    ax.axvline(phenology["peak_date"], color="red", linestyle="--", label="Peak", alpha=0.7)
    ax.axvline(
        phenology["senescence_date"], color="orange", linestyle="--", label="Senescence", alpha=0.7
    )
    ...
    # Add text annotation
    metrics_text = (
        f"Peak NDVI: {phenology['peak_ndvi']:.2f}\\n"
