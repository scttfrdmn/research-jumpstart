...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"Error processing ocean data: {e!s}")
...
def detect_anomalies(obs):
    ...
    if obs["dissolved_oxygen"] < DO_CRITICAL_THRESHOLD:
        ...
        status = "critical"
        alert_message += (
            f"SEVERE HYPOXIA: DO {obs['dissolved_oxygen']:.1f} mg/L at {obs['depth']:.0f}m depth. "
        )
...
"""
        ...
        print(f"Error sending SNS alert: {e!s}")
...
def store_in_dynamodb(obs):
    ...
    try:
        ...
        print(f"Error storing in DynamoDB: {e!s}")
        raise
