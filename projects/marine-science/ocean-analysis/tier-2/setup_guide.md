# AWS Setup Guide - Ocean Data Analysis Tier 2

This guide walks you through setting up all AWS resources needed for oceanographic data analysis. Estimated time: 35-45 minutes.
...
1. **S3 Bucket** - Store ocean data files
2. **IAM Role** - Lambda permissions
3. **Lambda Function** - Process oceanographic data
4. **DynamoDB Table** - Store ocean observations
5. **SNS Topic** - Send marine anomaly alerts
...
S3 stores your oceanographic data and serves as the entry point for the pipeline.

### Option A: Using AWS Console (Recommended for beginners)

1. Go to [AWS S3 Console](https://console.aws.amazon.com/s3/)
2. Click "Create bucket"
3. Enter bucket name: `ocean-data-{your-name}-{date}`
   - Example: `ocean-data-alice-20250114`
...
BUCKET_NAME="ocean-data-$(whoami)-$(date +%s)"
...
**Using AWS Console:**
1. Open your ocean-data bucket
2. Click "Create folder" → name it `raw` → Create
3. Click "Create folder" → name it `processed` → Create
4. Click "Create folder" → name it `logs` → Create
...
DynamoDB stores processed ocean observations for fast queries.

### Option A: Using AWS Console

1. Go to [DynamoDB Console](https://console.aws.amazon.com/dynamodb/)
2. Click "Create table"
3. Table name: `OceanObservations`
4. Partition key: `observation_id` (String)
...
aws dynamodb wait table-exists --table-name OceanObservations

# Verify table created
aws dynamodb describe-table --table-name OceanObservations \
...
```
OceanObservations
├── observation_id (Partition Key, String)     # "obs-20250114-001"
...
└── Attributes:
    ...
    ├── salinity (Number)                      # Practical Salinity Units (PSU)
    ├── ph (Number)                            # pH units
    ...
    ├── aragonite_saturation (Number)          # Ωarag (dimensionless)
    ├── primary_production (Number)            # mg C/m²/day
    ├── anomaly_status (String)                # "normal", "warning", "critical"
    ├── anomaly_type (String)                  # "marine_heatwave", "acidification"
...
SNS sends email alerts when marine anomalies are detected.
...
4. Type: **Standard**
5. Name: `ocean-anomaly-alerts`
6. Display name: `Ocean Anomaly Alerts`
...
```bash
# Create SNS topic
TOPIC_ARN=$(aws sns create-topic --name ocean-anomaly-alerts \
...
7. Click "Next"
8. Role name: `lambda-ocean-processor`
9. Description: `Lambda role for ocean data processing`
10. Click "Create role"

#### Get Role ARN:

1. Click on the role name `lambda-ocean-processor`
2. Copy the **ARN** at the top (you'll need this)
   - Format: `arn:aws:iam::123456789012:role/lambda-ocean-processor`
...
ROLE_ARN=$(aws iam get-role --role-name lambda-ocean-processor \
...
Lambda processes oceanographic data and triggers alerts.
...
4. Function name: `analyze-ocean-data`
5. Runtime: **Python 3.11**
6. Architecture: **x86_64**
7. Permissions: **Use an existing role**
   - Select `lambda-ocean-processor`
...
5. **Description**: Analyze ocean data and detect marine anomalies
6. Click "Save"
...
| Key | Value |
|-----|-------|
| `BUCKET_NAME` | `ocean-data-{your-bucket}` |
| `DYNAMODB_TABLE` | `OceanObservations` |
| `SNS_TOPIC_ARN` | `arn:aws:sns:us-east-1:ACCOUNT:ocean-anomaly-alerts` |
| `TEMPERATURE_ANOMALY_THRESHOLD` | `3.0` |
| `PH_WARNING_THRESHOLD` | `7.8` |
| `PH_CRITICAL_THRESHOLD` | `7.6` |
| `DO_WARNING_THRESHOLD` | `4.0` |
| `DO_CRITICAL_THRESHOLD` | `2.0` |
| `CHLOROPHYLL_BLOOM_THRESHOLD` | `20.0` |
...
cd /path/to/research-jumpstart/projects/marine-science/ocean-analysis/tier-2
...
aws lambda create-function \
  --function-name analyze-ocean-data \
  --runtime python3.11 \
  --role "$ROLE_ARN" \
...
aws lambda get-function --function-name analyze-ocean-data
```
...
Set up S3 to automatically invoke Lambda when new ocean data is uploaded.
...
1. Go to [Lambda Console](https://console.aws.amazon.com/lambda/)
2. Open `analyze-ocean-data` function
3. Click "Configuration" tab
4. Click "Triggers" in left menu
5. Click "Add trigger"
6. Select trigger: **S3**
7. Bucket: Select `ocean-data-{your-bucket}`
8. Event type: **All object create events**
9. Prefix: `raw/`
10. Suffix: `.csv` (to only process CSV files)
...
LAMBDA_ARN=$(aws lambda get-function --function-name analyze-ocean-data \
...
4. Set query result location: `s3://ocean-data-{your-bucket}/athena-results/`
5. Click "Save"
...
```sql
CREATE EXTERNAL TABLE IF NOT EXISTS ocean_observations (
  ...
  stratification_index DOUBLE,
  upwelling_index DOUBLE,
  aragonite_saturation DOUBLE,
...
LOCATION 's3://ocean-data-{your-bucket}/raw/'
  ...
  "Records": [
    {
      "s3": {
        "bucket": {
          "name": "ocean-data-{your-bucket}"
        },
        "object": {
          "key": "raw/test_ocean_data.csv"
        }
      }
    }
  ]
...
cat > test_ocean_data.csv <<EOF
timestamp,location_name,latitude,longitude,depth,temperature,salinity,ph,dissolved_oxygen,chlorophyll
2025-01-14T10:00:00Z,Gulf Stream,40.5,-70.2,0,22.5,35.8,8.05,7.2,2.5
2025-01-14T10:00:00Z,Gulf Stream,40.5,-70.2,50,18.3,35.9,8.02,6.8,1.8
2025-01-14T10:00:00Z,Gulf Stream,40.5,-70.2,100,12.1,35.5,7.98,5.5,0.9
EOF

# Upload to S3 (this should trigger Lambda)
aws s3 cp test_ocean_data.csv "s3://$BUCKET_NAME/raw/"
...
```bash
# View Lambda logs
aws logs tail /aws/lambda/analyze-ocean-data --follow
...
5. Budget name: `ocean-analysis-budget`
6. Period: **Monthly**
...
# Check IAM role
aws iam get-role --role-name lambda-ocean-processor
...
DYNAMODB_TABLE=OceanObservations
SNS_TOPIC_ARN=$TOPIC_ARN
LAMBDA_FUNCTION_NAME=analyze-ocean-data

# Ocean Anomaly Thresholds
TEMPERATURE_ANOMALY_THRESHOLD=3.0
PH_WARNING_THRESHOLD=7.8
PH_CRITICAL_THRESHOLD=7.6
DO_WARNING_THRESHOLD=4.0
...
BUCKET_NAME="ocean-data-$(whoami)-$(uuidgen | cut -d'-' -f1)"
aws s3 mb "s3://$BUCKET_NAME" --region us-east-1
```
...
# Should return: arn:aws:iam::ACCOUNT:role/lambda-ocean-processor
```
...
aws lambda get-policy --function-name analyze-ocean-data
...
4. **Analyze**: Open `notebooks/ocean_analysis.ipynb`
...
**Total setup cost: $0**
**Total running cost: $7-13 for full project**
...
1. Check CloudWatch logs: `aws logs tail /aws/lambda/analyze-ocean-data`
