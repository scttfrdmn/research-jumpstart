...
def plot_degree_distribution(
    ...
    figsize : tuple
        Figure size
    """
    fig, axes = plt.subplots(1, 2 if G.is_directed() else 1, figsize=figsize)
...
def create_interactive_dashboard(
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Network Graph', 'Degree Distribution',
                       'Top Influencers', 'Community Sizes'),
    ...
    fig.add_trace(
        go.Scatter(x=unique, y=counts, mode='markers', marker=dict(color='blue')),
        row=1, col=2
    )
    ...
    fig.add_trace(
        go.Bar(x=list(range(1, 21)), y=scores, marker=dict(color='coral')),
        row=2, col=1
    )
    ...
    if communities:
        ...
        fig.add_trace(
            go.Bar(x=list(range(1, len(sizes) + 1)), y=sizes, marker=dict(color='green')),
            row=2, col=2
        )

    # Update layout
    fig.update_layout(
        title_text="Social Network Analysis Dashboard",
        showlegend=False,
        height=900,
        width=1400
    )
