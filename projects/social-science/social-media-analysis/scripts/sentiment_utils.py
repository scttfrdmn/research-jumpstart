...
def extract_topics(texts, n_topics=5, method="lda", n_words=10, max_features=1000, min_df=2):
    """
    ...
    # Vectorize texts
    if method == "lda":
        vectorizer = CountVectorizer(max_features=max_features, min_df=min_df, stop_words="english")
        ...
        vectorizer = TfidfVectorizer(max_features=max_features, min_df=min_df, stop_words="english")
        doc_term_matrix = vectorizer.fit_transform(texts)
    ...
    for _topic_idx, topic in enumerate(model.components_):
...
def analyze_sentiment_by_topic(texts, sentiments, n_topics=5):
    ...
    df = pd.DataFrame({"topic": topic_assignments, "sentiment": sentiments})
...
if __name__ == "__main__":
    ...
    for i, (_text, score) in enumerate(zip(sample_texts, sentiments)):
